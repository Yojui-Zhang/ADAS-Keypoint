...
struct VirtualRoadSimulationOptions {
  ...
  bool disturbed_preview_mpc_enable = true; // 是否啟用受擾 Preview-MPC 對照控制器
...
struct AlgorithmAblationFrame {
  ...
  bool disturbed_preview_mpc_valid = false;         // 是否提供受擾 preview-MPC 對照
  double disturbed_preview_mpc_speed_kmh = 0.0;     // 受擾 preview-MPC speed 命令
  double disturbed_preview_mpc_steer_base_deg = 0.0;// 擾動注入前 preview-MPC steer
  double disturbed_preview_mpc_steer_deg = 0.0;     // 擾動注入後 steer 命令
  double disturbed_preview_mpc_brake_0_10 = 0.0;    // 受擾 preview-MPC brake 命令
  double steering_disturbance_deg = 0.0;            // 本幀注入的 deterministic steer disturbance
};
...
struct AlgorithmAblationResult {
  ...
  double disturbed_preview_mpc_cte_m = 0.0;          // 受擾 preview-MPC CTE（m）
  double disturbed_preview_mpc_heading_err_deg = 0.0;// 受擾 preview-MPC 航向誤差（deg）
  int disturbed_preview_mpc_lane_departure = 0;      // 受擾 preview-MPC 是否偏離車道（0/1）
...
private:
  ...
  struct Summary {
    ...
    uint64_t disturbed_preview_mpc_lane_departure_count = 0;
    ...
    double sum_abs_disturbed_preview_mpc_cte_m = 0.0;
    double max_abs_disturbed_preview_mpc_cte_m = 0.0;
    ...
    double sum_abs_disturbed_preview_mpc_heading_err_deg = 0.0;
    double max_abs_disturbed_preview_mpc_heading_err_deg = 0.0;
  ...
  bool has_virtual_sim_config_ = false;
  VirtualRoadSimulationOptions last_virtual_sim_config_;
  Summary summary_;
};
