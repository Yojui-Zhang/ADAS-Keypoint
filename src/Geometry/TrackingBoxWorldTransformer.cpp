...
std::vector<cv::Point3f> TrackingBoxWorldTransformer::toWorldPoints(TrackingBox& tb) const
{
    ...
    tb.World_box.clear();
    tb.World_box.reserve(2);

    cv::Point3f Xw_bl, Xw_br;
    bool ok1 = projector_.pixelToWorldOnPlane(bl, Xw_bl);
    bool ok2 = projector_.pixelToWorldOnPlane(br, Xw_br);

    tb.World_box.push_back(ok1 ? Xw_bl : WorldProjector::NaNPoint());
    tb.World_box.push_back(ok2 ? Xw_br : WorldProjector::NaNPoint());
