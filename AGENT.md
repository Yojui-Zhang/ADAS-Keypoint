...
## 32. 本輪再追加（逐幀完整 AI / SORT / IPM 偵測值落盤）

### 32.1 本輪用戶要求（分類）
1. 在保留現有 research log 資訊前提下，新增每一幀 AI detect 出的所有物件數值。
2. 每個物件需包含影像座標 bbox 與所有 keypoints。
3. 同時記錄 raw detect、IPM/world 轉換後座標、SORT filter 後座標。
4. 若每組 keypoints 為 15 點，需能記錄 raw 15 點、SORT 15 點、IPM 15 點，合計 45 點。

### 32.2 思考與決策摘要（可公開版本）
- **決策 BI：主 research CSV 維持相容，新增 companion detail CSV**
  - 原因：逐物件 keypoints 是可變長資料，直接塞進原主 CSV 會破壞既有分析欄位與腳本。
  - 做法：保留 `research_drive_*.csv`，新增 `research_drive_*.detections.csv`。

- **決策 BJ：SORT row 優先同列保存 raw / SORT / IPM 三組 keypoints**
  - 原因：用戶要求同一組 keypoints 需包含 45 點。
  - 做法：SORT 輸出新增 `source_detection_index`，可配對回 raw detection 時，同一列寫入 `raw_kpts_px`、`sort_kpts_px`、`ipm_kpts_world_m`。

- **決策 BK：不可靠配對不硬湊**
  - 原因：SORT 可能因 min_hits、max_age、predict-only 或 unmatched 狀態導致沒有當幀 raw detection。
  - 做法：無配對時 raw 欄位留空，但仍保存 SORT/IPM 資料；raw detection 另有 `stage=raw_detect` 逐物件完整落盤。
...
- 引擎 raw detection 輸出：
  ...
  - `Engine/TensorRT/include/TensorRT_main.hpp`
  - `Engine/TensorRT/src/TensorRT_main.cc`
- Geometry 無繪圖轉換 helper：
  - `include/Geometry/GeometryFunction.h`
  - `src/Geometry/GeometryFunction.cpp`
- SORT 配對索引：
  - `include/config.h`
  - `src/SORT/SortTracking.cc`
- Log 接線與落盤：
  - `include/log/runtime_log_manager.h`
  - `src/log/runtime_log_manager.cpp`
  - `include/log/research_data_logger.h`
...
### 32.4 新增輸出欄位重點
- `research_drive_*.detections.csv`
  - `stage`
  - `object_index`
  - `source_detection_index`
  - `track_id`
  - `class_id/class_label/score/classify_num`
  - `box_x_px/box_y_px/box_w_px/box_h_px`
  - `raw_kpts_px`
  - `sort_kpts_px`
  - `world_box_m`
  - `ipm_kpts_world_m`
...
- 建置驗證：
  - `cmake --build build-TFlite -j4`：通過
- 執行入口驗證：
  - `./build-TFlite/ADAS` 無參數可正常顯示 usage。
